...
# Temporary audio file with .wav extension
...
ffmpeg -y -f concat -safe 0 -i "$file_list" -c copy "$temp_audio"

# Create video
ffmpeg -y -i "$temp_audio" -filter_complex "[0:a]showwaves=s=1280x720:mode=line,format=yuv420p[v]" -map "[v]" -map 0:a -c:v libx264 -c:a aac "$output_file"
